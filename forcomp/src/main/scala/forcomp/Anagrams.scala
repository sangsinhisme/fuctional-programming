...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(identity).view.mapValues(_.length).toList.sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match
    ...
    case (char, count) :: tail =>
      ...
      yield if n == 0 then rest else (char, n) :: rest).toList
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    y.foldLeft(x.toMap) { case (acc, (char, count)) =>
        ...
        if newCount > 0 then acc.updated(char, newCount)
        else acc - char
    }.toList.sortBy(_._1)
    ...
    def loop(occurrences: Occurrences): List[Sentence] =
      if occurrences.isEmpty then List(Nil)
        ...
        for {
          ...
          rest <- loop(subtract(occurrences, comb))
        } yield word :: rest
