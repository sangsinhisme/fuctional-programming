...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet =
    ...
    val maxChild = if leftMost.retweets > rightMost.retweets then leftMost else rightMost
    if elem.retweets > maxChild.retweets then elem else maxChild

  def descendingByRetweet: TweetList  = {
    val top = mostRetweeted
    val tail = this.remove(top)

    Cons(top, tail.descendingByRetweet)
  }
  /**
   * The following methods are already implemented
   */
